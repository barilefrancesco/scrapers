...
from telethon.tl.types import InputPeerEmpty, ChatForbidden, ChannelForbidden
import json
...
def telegram_messages():
    ...
    for chat in chats:
        try:
            if (not isinstance(chat, ChatForbidden)) and (not isinstance(chat, ChannelForbidden)) and  (chat.participants_count != 0):
                groups.append(chat)
        except Exception as e:
            print(e)
            continue
    ...
    for i in range(len(groups)):
        try:
            ...
            continue
    client.disconnect()
    return json.dumps(messages_JSONS,sort_keys=True)
